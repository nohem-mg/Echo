...
Bun.serve({
  ...
  async fetch(req) {
    ...
    if (pathname === "/api/registry" && req.method === "POST") {
      ...
      // track_id is now generated by the registry service; CRE no longer supplies it.
      if (payload.midiSequence == null) {
        return json({ code: "validation_error", message: "midiSequence required" }, 422);
      }
      if (!(await healthy(REGISTRY_URL))) {
        vlog("← registry MOCK (registry-service down)");
        return json(mockRegister());
      }
      try {
        return json(
    ...
    if (pathname === "/api/report" && req.method === "POST") {
      // AgentKit payment gate — only enforced when ECHO_REQUIRE_AGENTKIT=1.
      // In dev/simulation the CRE does not carry a human credential header.
      const requireAgentkit = process.env.ECHO_REQUIRE_AGENTKIT === "1";
        ...
        return Response.json({
          ...
          extensions: {
            agentkit: {
              ...
              supportedChains: [{ chainId: "eip155:4801", type: "eip191", signatureScheme: "eip191" }]
            }
          }
        }, {
          status: 402,
      ...
      if (requireAgentkit && agentkitHeader) {
        try {
          ...
          const validation = await validateAgentkitMessage(parsed, resourceUri, { maxAge: 3_600_000 });
          if (!validation.valid) throw new Error(validation.error);

          const verification = await verifyAgentkitSignature(parsed, process.env.ECHO_RPC_URL);
          ...
          nullifier = humanId;
          vlog("AgentKit verification successful — address:", verification.address, "humanId:", humanId.slice(0, 12) + "…");
        } catch (err: any) {
        ...
        // Trial mechanic (only when AgentKit is active).
        const trialKey = nullifier!;
        const trialCount = reportTrialCounts.get(trialKey) || 0;
        if (trialCount >= 3) {
          vlog(`Trial exhausted for nullifier ${trialKey} (count=${trialCount})`);
          return json({ code: "payment_required", message: "Trial épuisé — paiement requis" }, 402);
        }
        reportTrialCounts.set(trialKey, trialCount + 1);
        vlog(`Trial count for nullifier ${trialKey} incremented to ${trialCount + 1}`);
      }
